...
### 1Week-Day3
1. 동작 구현에 관련된 매서드를 SayingControl로 이동

App 클래스에서 명언앱을 실행 후 발생하는 모든 동작들에 관련된 매서드를 모두 SayingControl 클래스로 옮겼습니다.

이 때 makeJson 매서드에서 리스트로 받아오던 매개변수를 없애주고 this 매서드를 이용해서 List<Saying> sayings=this.sayings; 로 구현 했습니다.

처음에 이 과정을 안했더니 json 파일에 저장이 되지 않았고 왜 그런가 살펴보니 this.sayings을 해주지 않아서 리스트를 제대로 가져오지 못하고 있어서 그런 것 이었습니다.

두번째로, 실행을 여러번 할 때 마지막 인덱스 값을 불러오는 동작을 하나의 매서드로 구현 했습니다.

이전에는 App 클래스에서 반복문에 들어가기 전 if문을 통해 마지막 인덱스 값을 가져왔었는데 그 동작을 매서드로 구현해서 SayingControl에 추가하게 되었습니다.

이렇게 해서 App 클래스가 보다 가독성 좋게 정리 될 수 있었습니다.

2. 롬복으로 Getter / Setter 구현하기

롬복을 사용해 Saying 클래스의 3가지 속성들에 대해 private 설정을 주고 Getter / Setter를 구현 했습니다.

그리고 이 속성들이 참조되는 매서드에서는 saying.idx가 아닌 saying.getIdx() 형식으로 모두 변경 해 주었습니다.


3. json 파일이 비어있을때 발생하는 오류 메세지 변경

json 파일이 비어있을때 e.printStackTrace(); 를 통해서 예외 발생을 추적하는 메세지가 발생 하도록 구현을 했었습니다.
이 경우 메세지는 발생 하지만 다음 동작의 실행은 정상적으로 진행 되었습니다.

프로그램 실행에 문제는 없었으나 예외처리 메세지가 길고 복잡하게 나와 마치 오류가 있는 것 처럼 보여지는 상황 이었기 때문에 
json 파일이 비어있을 때 "Json 파일에 저장된 데이터가 없습니다." 라고 출력 해 줘서
프로그램 실행 창이 보다 깔끔 해 지도록 구현했습니다.

이렇게 해서 json 파일 안의 내용을 모두 지우고 실행을 해도 문제가 없게 되었습니다.

여기까지 해서 이번 미션 프로젝트를 마치게 되었습니다.


**[특이사항]**

1. 커밋명을 수정하는 과정에서 불필요한 커밋을 추가하게 된 점이 아쉽습니다.
2. git에 push 하기 전에 커밋명에 오류가 있는지 확인 해보지 않고 push를 한 점이 아쉬웠습니다.
3. 텍스트 파일로 저장하는 과정을 구현 할 때 디버깅 기능을 먼저 사용 해봤더라면 문제를 금방 파악 할 수 있었지 않았을까 하는 아쉬움이 있습니다.
4. json 파일과 txt 파일 동시에 사용 해도 되는지에 대한 궁금증이 있었으나 해결 되었습니다.


- 추후 리팩토링 시, 어떤 부분을 추가적으로 진행하고 싶은지에 대해 구체적으로 작성해주시기 바랍니다.

1. 리팩토링 시 mvc 모델 대로 클래스를 구분 해 보고 싶습니다. 아직 제 머릿속에 정확한 개념이 잡히지 않아서 이번에는 구현하지 못했습니다.
2. 불 필요한 코드들이 중복 되지 않았는지 더 면밀하게 검토 해보고 싶습니다.


### 리팩토링

-[x] 삭제 / 수정 시 json 파일을 업데이트 하는 과정 구현
-[x] 빌드 명령어를 생성, 빌드 명령어 입력 시 json 파일이 업데이트 되도록 구현
-[x] 폴더를 세분화 하여 수업시간에 배운대로 리팩토링
-[x] json 파일을 resources 파일로 옮겼기 때문에 SayingControl에서 불러올 때 경로 수정
